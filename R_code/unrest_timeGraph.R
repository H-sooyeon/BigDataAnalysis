...
# 색상 벡터 지정
category_colors <- c("#B2182B", "#D6604D", "#F4A582", "black", "#FDDBC7", "#92C5DE", "#4393C3", "#2166AC")

# Step 5: Plot time series graph for each category
ggplot(data_long, aes(x = year_month, y = value, color = 구분)) +
  geom_line(size = 1) +
  labs(x = "\nYear", y = "Value\n", title = "                                    20-22년 분기별 불안 (GAD-7) 시계열 그래프\n") +
